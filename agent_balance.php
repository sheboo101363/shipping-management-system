...
// جلب قائمة الوكلاء وأرصدة كل وكيل مع التصفية والبحث
$where = "WHERE 1";
if($search != '') {
    $safe_search = mysqli_real_escape_string($conn, $search);
    $where .= " AND (a.company_name LIKE '%$safe_search%' OR a.id LIKE '%$safe_search%')";
}
...
$q = "
SELECT a.id, a.company_name, IFNULL(SUM(IF(t.direction='in', t.amount, -t.amount)),0) as balance,
    COUNT(t.id) as trans_count,
    MAX(t.created_at) as last_trans
FROM agents a
LEFT JOIN cashbox_transactions t ON t.type='agent' AND t.relation_id=a.id
$where
GROUP BY a.id
$having
...
// تفاصيل حركات الوكيل عند الطلب (AJAX)
if(isset($_GET['agent_id']) && is_numeric($_GET['agent_id'])) {
    ...
    $history_q = "
        ...
        WHERE type='agent' AND relation_id=$agent_id
    ...
    <div class="table-responsive mt-3">
        <table class="table table-bordered table-striped text-end align-middle">
            ...
            <tbody>
                ...
                <tr>
                    <td colspan="5" class="text-center text-muted">لا يوجد حركات لهذا الوكيل</td>
                </tr>
                <?php endif; ?>
            </tbody>
            <?php if($totals): ?>
    ...
    <div class="mt-4 text-center">
        <button class="btn btn-outline-primary" onclick="printDiv('agent-history')"><i class="fa-solid fa-print"></i> طباعة الكشف</button>
        <button class="btn btn-outline-success" onclick="exportTableToExcel('agent-history')"><i class="fa-solid fa-file-excel"></i> تصدير Excel</button>
    </div>
    <script>
    function printDiv(divId){
        ...
        frame.contentDocument.write('<html><head><title>كشف حساب الوكيل</title></head><body>' + contents + '</body></html>');
...
// قسم الجدول للوكلاء (عند الطلب AJAX)
if(isset($_POST['ajax_agents'])) {
    ob_clean();
    ?>
    <div class="table-responsive sticky-header">
        <table class="table table-bordered table-striped text-end align-middle">
            ...
            <tbody>
                ...
                <tr class="<?= $rowClass ?>">
                    ...
                    <td>
                        <button type="button"
                            ...
                            onclick="showHistory(<?= $row['id'] ?>,'<?= htmlspecialchars($row['company_name'],ENT_QUOTES) ?>')">
                ...
                <?php if($row_number==1): ?>
                <tr>
                    <td colspan="7" class="text-center">لا يوجد وكلاء</td>
                </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
<head>
    <meta charset="utf-8">
    <title>كشف أرصدة الوكلاء | الخزنة</title>
    ...
    <style>
        ...
        .section-title {font-weight:bold; font-size:1.35em; color:#d2a017; margin-bottom:24px; border-bottom:2px solid #fffbe3; padding-bottom:12px;}
        .tooltip-icon {color:#d2a017; cursor:help;}
        .table-responsive {border-radius:18px;}
        .table thead th {background:#fffbe3; color:#ba8b00;}
        ...
        .row-number {font-weight:bold; color:#d2a017;}
        .btn-details {font-size:0.95em; border-radius:1.3rem;}
        .history-section {background:#fff; border-radius:18px; box-shadow:0 2px 14px #fffbe3; margin-bottom:32px; padding:28px 20px 18px 20px;}
        .sticky-header thead th {position:sticky; top:0; background:#fffbe3;}
        .negative-row {background:#ffeeee;}
        .filter-area {background:#fffbe3; border-radius:12px; padding:16px 14px; margin-bottom:24px;}
...
<div class="container py-4" style="direction:rtl;">
    ...
    <div class="mb-3 text-start">
        <a href="main_vault.php" class="btn btn-warning main-btn"><i class="fa-solid fa-vault"></i> الخزنة الرئيسية</a>
    </div>
    ...
    <div class="filter-area" id="filter-area">
        <form id="filterForm" class="row g-2 align-items-center" autocomplete="off">
            <div class="col-md-4 mb-2">
                <input type="text" id="searchAgent" name="search" class="form-control" placeholder="ابحث باسم الشركة أو الرقم التعريفي...">
            </div>
            ...
            <div class="col-md-2 mb-2">
                <button type="submit" class="btn btn-warning main-btn"><i class="fa-solid fa-filter"></i> تصفية</button>
            </div>
            <div class="col-md-3 mb-2 text-end">
                ...
                <button type="button" class="btn btn-outline-success main-btn" onclick="exportTableToExcel('agents-table-area')"><i class="fa-solid fa-file-excel"></i> تصدير قائمة الوكلاء</button>
                <button type="button" class="btn btn-outline-primary main-btn" onclick="printDiv('agents-table-area')"><i class="fa-solid fa-print"></i> طباعة القائمة</button>
            </div>
        </form>
    </div>

    <!-- كشف أرصدة الوكلاء -->
    <div class="section-card" id="agents-table-area">
        <div class="section-title">
            <i class="fa-solid fa-user-tie"></i> كشف أرصدة الوكلاء
        </div>
        <div id="agents-table-content">
            <!-- الجدول يظهر هنا -->
            <div class="table-responsive sticky-header">
                <table class="table table-bordered table-striped text-end align-middle">
                    <thead>
    ...
    <!-- كشف حساب الوكيل -->
    <div class="history-section" id="agent-history-section" style="display:none;">
        <div class="section-title" id="history-title"><i class="fa-solid fa-file-lines"></i> كشف حساب الوكيل</div>
...
function updateAgentsTable(data){
    ...
    xhr.open('POST', 'agent_balance.php', true);
    xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');
    xhr.onload = function(){
        document.getElementById('agents-table-content').innerHTML = this.responseText;
    };
    var formData = new FormData();
    for(var key in data){ formData.append(key, data[key]); }
    formData.append('ajax_agents', '1');
    xhr.send(formData);
}
...
document.getElementById('resetBtn').addEventListener('click', function(e){
    ...
    updateAgentsTable({search:'',filter_balance:'all'});
});

// دعم الفلترة والبحث بدون إعادة تحميل الصفحة
document.getElementById('filterForm').addEventListener('submit', function(e){
    e.preventDefault();
    var search = document.getElementById('searchAgent').value;
    var balance = document.getElementById('filter_balance').value;
    updateAgentsTable({search:search,filter_balance:balance});
});

function showHistory(id, name){
    document.getElementById('agent-history-section').style.display = 'block';
    document.getElementById('history-title').innerHTML = '<i class="fa-solid fa-file-lines"></i> كشف حساب: ' + name;
    document.getElementById('agent-history').innerHTML = '<div class="text-center p-3"><i class="fa-solid fa-spinner fa-spin fa-2x text-warning"></i><br>جاري التحميل...</div>';
    var xhr = new XMLHttpRequest();
    xhr.open('GET', 'agent_balance.php?agent_id='+id, true);
    xhr.onload = function(){
        document.getElementById('agent-history').innerHTML = '<div id="agent-history">' + this.responseText + '</div>';
    };
    xhr.send();
}
function hideHistory(){
    document.getElementById('agent-history-section').style.display = 'none';
}
...
function exportTableToExcel(divId){
    ...
    var a = document.createElement('a');
    a.href = 'data:application/vnd.ms-excel,' + html;
    a.download = 'كشف_ارصدة_الوكلاء.xls';
    a.click();
}
...
function printDiv(divId){
    ...
    win.document.write('<html><head><title>كشف أرصدة الوكلاء</title>');
